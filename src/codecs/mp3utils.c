...
static void riff_read_meta_field(Mix_MusicMetaTags *tags, Mix_MusicMetaTag tag_type, size_t *i, Uint32 chunk_length, Uint8 *data, size_t fieldOffset)
{
    Uint32 len = 0;
...
static SDL_bool riff_parse_LIST(Mix_MusicMetaTags *out_tags, struct mp3file_t *wave, Uint32 chunk_length)
{
    Uint8 *data;
    Uint32 got;
    ...
    if (SDL_strncmp((char *)data, "INFO", 4) == 0) {
        ...
        for (i = 4; i < got - 4;) {
            if(SDL_strncmp((char *)(data + i), "INAM", 4) == 0) {
                riff_read_meta_field(out_tags, MIX_META_TITLE, &i, got, data, 4);
                continue;
            } else if(SDL_strncmp((char *)(data + i), "IART", 4) == 0) {
                riff_read_meta_field(out_tags, MIX_META_ARTIST, &i, got, data, 4);
                continue;
            } else if(SDL_strncmp((char *)(data + i), "IALB", 4) == 0) {
                riff_read_meta_field(out_tags, MIX_META_ALBUM, &i, got, data, 4);
                continue;
            } else if (SDL_strncmp((char *)(data + i), "BCPR", 4) == 0) {
                riff_read_meta_field(out_tags, MIX_META_COPYRIGHT, &i, got, data, 4);
                continue;
            }
            i++;
        }
...
static int probe_riff_container(Mix_MusicMetaTags *out_tags, struct mp3file_t *fil, Uint8 *buf)
{
    Uint32 magic;
    Uint32 chunk_type;
    Uint32 chunk_length;
    ...
    while(1) {
        ...
        chunk_type = SDL_SwapLE32(*(Uint32*)chunk);
        chunk_length = SDL_SwapLE32(*(Uint32*)(chunk + 4));
        ...
        case 0x20746D66: /* fmt */
            ...
            if (MP3_RWseek(fil, chunk_length - 2, RW_SEEK_CUR) < chunk_length - 2) {
                return TAG_NOT_FOUND;
            }
            break;
        ...
        if (chunk_length & 1) {
            if (MP3_RWseek(fil, 1, RW_SEEK_CUR) < 1) {
                return TAG_NOT_FOUND;
            }
        }
    }
...
int mp3_read_tags(Mix_MusicMetaTags *out_tags, struct mp3file_t *fil, SDL_bool keep_id3v2)
{
    ...
    if (probe_riff_container(out_tags, fil, buf)) {
        goto succ;
    }
