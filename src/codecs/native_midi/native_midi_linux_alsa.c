...
{
    const char *ret = SDL_GetHint(SDL_HINT_APP_NAME);
    if (ret && *ret)
        return ret;
...
void native_midi_pause(void)
{
    ...
    if (!song || SDL_AtomicGet(&song->playerstate) == NATIVE_MIDI_STOPPED)
        return;
...
void native_midi_resume(void)
{
    ...
    if (!song || SDL_AtomicGet(&song->playerstate) != NATIVE_MIDI_PAUSED)
        return;
